...
func Test_rekord_certs(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name      string
...
func Test_rekord_matches(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name     string
		blobPath string
		...
		{
			name:     "valid match",
			blobPath: "./testdata/rekor/uploaded-blob.md",
			rekord: hashedRekordBody{
				Spec: hashedRekordSpec{
					Data: hashedRekordData{
						Hash: map[string]string{
